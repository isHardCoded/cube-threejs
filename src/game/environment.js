...
export function createEnvironment(canvas, mapId) {
  ...
  function updateCamera(dt, t, focus) {
    ...
    // Freeroam sticks tight; Duel Run / explicit follow overrides classic lag.
    const tight = !!focus?.tight
    const follow = focus?.follow != null ? focus.follow : (tight ? 1 : 0.55)
    const lookFollow = focus?.lookFollow != null ? focus.lookFollow : (tight ? 1 : Math.min(1, follow + 0.05))

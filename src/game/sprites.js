...
// Nameplate: nickname + HP bar + remaining lives + dash strip, all on one sprite
// above the die.
...
// Lives as little cubes: knowing an enemy is on their last one is half the game.
function drawLives(ctx, lives, maxLives) {
  const size = 9
  const gap = 4
  const total = maxLives * size + (maxLives - 1) * gap
  let x = (PLATE_W - total) / 2
  for (let i = 0; i < maxLives; i++) {
    const left = i < lives
    ctx.fillStyle = left ? '#2ee66b' : 'rgba(255,255,255,.16)'
    ctx.fillRect(x, 44, size, size)
    if (left) {
      ctx.strokeStyle = 'rgba(0,0,0,.5)'
      ctx.lineWidth = 1
      ctx.strokeRect(x + 0.5, 44.5, size - 1, size - 1)
    }
    x += size + gap
  }
}
...
export function drawNameplate(bar, hp, dashFrac = null, lives = null, maxLives = 5) {
  const { ctx, tex } = bar
  const w = PLATE_W
  ...
  if (lives !== null) drawLives(ctx, lives, maxLives)

  if (dashFrac !== null) {
    ctx.fillStyle = 'rgba(5,5,12,.8)'
    ctx.fillRect(16, 58, w - 32, 8)
    ctx.fillStyle = dashFrac >= 1 ? '#fcee0a' : 'rgba(252,238,10,.55)'
    ctx.fillRect(18, 60, (w - 36) * Math.min(1, dashFrac), 4)
    ctx.strokeStyle = 'rgba(252,238,10,.6)'
    ctx.lineWidth = 1
    ctx.strokeRect(16.5, 58.5, w - 33, 7)
  }
